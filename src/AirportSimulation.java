...
public class AirportSimulation {
    ...
    public static void main(String[] args) {
        ...
        System.out.print("Time to land: ");
        timeToLand = scanner.nextInt();
        System.out.print("Time to takeoff: ");
        timeToTakeoff = scanner.nextInt();
        System.out.print("Average time between landing planes: ");
        averageTimeLanding = scanner.nextInt();
        System.out.print("Average time between taking off planes: ");
        averageTimeTakingOff = scanner.nextInt();
        System.out.print("Max time in queue: ");
        maxTimeInQueue = scanner.nextInt();
        System.out.print("Total time to simulate: ");
        ...
        probabilityToTakeoff = scanner.nextInt();
        System.out.println();
        ...
        System.out.println("(3) The average amount of time between arrival of planes to the landing queue = " + averageTimeLanding);
        System.out.println("(4) The average amount of time between arrival of planes to the takeoff queue = " + averageTimeTakingOff);
        System.out.println("(5) The maximum amount of time that a plane can stay in the landing queue without running out of fuel and crashing = " + maxTimeInQueue);
        ...
        for (int now = 1; now <= totalTime; now++) {
            ...
            while ( !queueLanding.isEmpty()          // the landing queue is not empty
                    && queueLanding.peek() <= now) { // the ready time of the first plane in the landing queue before now
                ...
                if (now - planeReadyTime > maxTimeInQueue) {
                    ...
                    queueLanding.remove();
                    numCrashed++;
                    System.out.println("!!!!    The plane in Landing Queue at #" + planeReadyTime + " minute is crashed.");
                    System.out.println();
                    continue;
                }
                ...
                if (totalTime - now > timeToLand) {
                    ...
                    waitingTime += timeToLand - 1; // the waiting time starts from this minute so the remaining time should minus one
                    System.out.println("        The plane in Landing Queue at #" + planeReadyTime + " minute is landed.");
                    System.out.println();
            ...
            System.out.println("    The number of planes that crashed because they ran out of fuel before they could land = " + numCrashed);
            ...
            if (waitingTime <= 0                      // waitingTime < 0 means there is no plane landing
                ...
                && totalTime - now > timeToTakeoff) { // there is enough time for the plane to take off during the simulation

                planeReadyTime = queueTakeoff.remove();

                numTookOff++;
                totalTimeTakingOff += now - planeReadyTime;
                waitingTime += timeToTakeoff - 1; // the waiting time starts from this minute so the remaining time should minus one
        ...
        System.out.println("(3) The number of planes that crashed because they ran out of fuel before they could land = " + numCrashed);
        System.out.println("(4) The average time a plane spent in the takeoff queue = " + (double)totalTimeTakingOff / numTookOff);
        System.out.println("(5) The average time that a plane spent in the landing queue = " + (double)totalTimeLanding / numLanded);
    }
}
